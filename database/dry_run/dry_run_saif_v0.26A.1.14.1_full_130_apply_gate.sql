...
SELECT 'PLANNER_GATE' AS section,
       ...
       SUM(p.decision_code='baseline_ready' AND p.apply_status='draft') AS baseline_expected_68,
       SUM(p.decision_code='progression_optional' AND p.apply_status='draft') AS progression_expected_62,
       SUM(p.decision_code NOT IN ('baseline_ready','progression_optional')) AS deferred_expected_81,
       SUM(p.decision_code IN ('baseline_ready','progression_optional')) AS selected_expected_130,
       SUM(p.decision_code IN ('baseline_ready','progression_optional') AND (p.runtime_modelid<400 OR p.runtime_modelid>611)) AS invalid_model_should_be_zero,
       SUM(p.decision_code IN ('baseline_ready','progression_optional') AND ABS(q.pos_x)<0.001 AND ABS(q.pos_y)<0.001 AND ABS(q.pos_z)<0.001) AS zero_coordinate_should_be_zero,
       SUM(p.decision_code IN ('baseline_ready','progression_optional') AND (p.requires_model_resolution<>0 OR p.requires_state_bridge<>0)) AS unresolved_dependency_should_be_zero
