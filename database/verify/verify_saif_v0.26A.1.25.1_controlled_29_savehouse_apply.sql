...
-- Verify latest live controlled GTA SA 29-savehouse apply and allocator-managed map icon budget.
...
SET @verify_public_icons := (SELECT COUNT(*) FROM public_interiors WHERE enabled=1 AND exterior_map_icon>0);
SET @verify_hospital_icons := (SELECT COUNT(*) FROM public_interiors WHERE enabled=1 AND interior_type='hospital' AND exterior_map_icon>0);
SET @verify_hospital_fallback := GREATEST(0,7-LEAST(7,@verify_hospital_icons));
SET @verify_public_candidates := @verify_public_icons+@verify_hospital_fallback;
SELECT 'MAP_ICON_ALLOCATOR_GATE' section,
       @verify_public_icons public_db_icon_candidates,
       @verify_hospital_fallback hospital_fallback_candidates,
       @verify_public_candidates total_public_candidates,
       LEAST(91,@verify_public_candidates) public_icons_rendered_max_91,
       GREATEST(0,@verify_public_candidates-91) public_icons_omitted_by_allocator_informational,
       (SELECT COUNT(*) FROM house_map_icon_policy WHERE id=1 AND enabled=1 AND public_service_slots=91 AND owned_house_slots=1 AND nearby_for_sale_slots=8) allocator_policy_ready_should_be_1,
       9 protected_house_slots;
