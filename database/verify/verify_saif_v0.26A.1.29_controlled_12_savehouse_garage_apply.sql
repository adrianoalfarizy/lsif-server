...
SELECT 'HOUSE_LINK_GATE' section,
       ...
       COUNT(DISTINCT garage_catalog_id) unique_garages_expected_12,
       COUNT(DISTINCT canonical_house_slot) unique_slots_expected_12,
       SUM(enabled=1) enabled_links_expected_12,
       SUM(link_class<>'baseline_savehouse') wrong_link_class_should_be_zero,
       SUM(ownership_mode<>'inherit_house_owner') wrong_ownership_mode_should_be_zero,
       SUM(access_mode<>'house_owner_only') wrong_access_mode_should_be_zero,
