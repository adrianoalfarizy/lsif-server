...
forward OnGangRespectLoaded(playerid);
...
stock ResetPlayerGangData(playerid)
{
    ...
    PlayerInsideGangHQ[playerid] = 0;
    PlayerInsideGangHQID[playerid] = 0;
    PlayerLastGangHQPickupTick[playerid] = 0;
...
stock LoadPlayerGang(playerid)
{
    ...
    mysql_format(
        ...
        query,
        sizeof(query),
        "SELECT gm.gang_id, gm.rank_level, g.name, g.gang_color, COALESCE(gs.respect,0) AS respect, COALESCE(gs.captures,0) AS captures, COALESCE(gs.defends,0) AS defends, COALESCE(gs.wars_participated,0) AS wars_participated FROM gang_members gm JOIN gangs g ON g.id = gm.gang_id LEFT JOIN gang_member_stats gs ON gs.gang_id=gm.gang_id AND gs.player_id=gm.player_id WHERE gm.player_id=%d LIMIT 1",
        PlayerDBID[playerid]
    );
...
stock ProcessJoinPresetGang(playerid, gangid)
{
    ...
    new statQuery[384];
    mysql_format(g_SQL, statQuery, sizeof(statQuery), "INSERT INTO gang_member_stats (gang_id, player_id, player_name) VALUES (%d, %d, '%e') ON DUPLICATE KEY UPDATE player_name=VALUES(player_name), updated_at=NOW()", gangid, PlayerDBID[playerid], playerName);
    mysql_tquery(g_SQL, statQuery);

    PlayerGangID[playerid] = gangid;
    PlayerGangRank[playerid] = GANG_RANK_MEMBER;
    PlayerGangRespect[playerid] = 0;
    PlayerGangCaptures[playerid] = 0;
    PlayerGangDefends[playerid] = 0;
    PlayerGangWars[playerid] = 0;
...
stock ResetTurfWarData()
{
    for (new i = 0; i < MAX_TERRITORIES; i++)
    {
        TurfWarState[i] = TURF_WAR_STATE_NONE;
        TurfWarAttackerGangID[i] = 0;
        TurfWarDefenderGangID[i] = 0;
        TurfWarCaptureSecondsLeft[i] = 0;
        TurfWarGraceSecondsLeft[i] = 0;
        TurfWarCooldownUntil[i] = 0;
    }

    for (new p = 0; p < MAX_PLAYERS; p++)
    {
        PlayerTurfHoldTerritory[p] = -1;
        PlayerTurfHoldGang[p] = 0;
        PlayerTurfHoldActive[p] = 0;
        PlayerTurfHoldStarted[p] = 0;
    }

    return 1;
}

stock IsPlayerInsideTerritory(playerid, territoryIndex)
{
    if (!IsPlayerConnected(playerid) || !IsValidTerritoryIndex(territoryIndex) || !TerritoryEnabled[territoryIndex]) return 0;

    new Float:x, Float:y, Float:z;
    GetPlayerPos(playerid, x, y, z);
    NormalizeTerritoryCorners(territoryIndex);

    if (x >= TerritoryMinX[territoryIndex] && x <= TerritoryMaxX[territoryIndex] && y >= TerritoryMinY[territoryIndex] && y <= TerritoryMaxY[territoryIndex]) return 1;
    return 0;
}
...
stock GetPlayerCurrentTerritory(playerid)
{
    for (new i = 0; i < MAX_TERRITORIES; i++)
    {
        if (IsPlayerInsideTerritory(playerid, i)) return i;
    }
    return -1;
}

stock CountGangMembersInsideTerritory(gangid, territoryIndex)
{
    ...
    for (new i = 0; i < MAX_PLAYERS; i++)
    {
        ...
        if (IsPlayerInsideTerritory(i, territoryIndex)) count++;
    }
    return count;
}

stock SendTurfWarMessage(territoryIndex, color, const message[])
{
    if (!IsValidTerritoryIndex(territoryIndex)) return 0;
    new attacker = TurfWarAttackerGangID[territoryIndex];
    new defender = TurfWarDefenderGangID[territoryIndex];
    ...
    {
        if (!IsPlayerConnected(i) || !PlayerLoggedIn[i]) continue;
        if (PlayerGangID[i] == attacker || PlayerGangID[i] == defender) SendClientMessage(i, color, message);
    }
    return 1;
}

stock GetGangNameByID(gangid, gangName[], size)
{
    new gangIndex = GetPresetGangIndexByID(gangid);
    if (gangIndex == -1)
    {
        format(gangName, size, "Neutral");
        return 0;
    }
    format(gangName, size, "%s", PresetGangName[gangIndex]);
    return 1;
}

stock CanPlayerChallengeTurf(playerid, territoryIndex)
{
    if (!IsPlayerConnected(playerid) || !PlayerLoggedIn[playerid]) return 0;
    if (PlayerGangID[playerid] <= 0)
    {
        SendClientMessage(playerid, COLOR_RED, "Kamu harus tergabung dalam gang untuk menantang turf.");
        return 0;
    }
    if (!IsValidTerritoryIndex(territoryIndex) || !TerritoryEnabled[territoryIndex])
    {
        SendClientMessage(playerid, COLOR_RED, "Kamu tidak berada di area turf yang valid.");
        return 0;
    }
    if (!IsPlayerInsideTerritory(playerid, territoryIndex))
    {
        SendClientMessage(playerid, COLOR_RED, "Kamu harus berada di dalam area turf untuk challenge.");
        return 0;
    }
    if (TerritoryOwnerGangID[territoryIndex] == PlayerGangID[playerid])
    {
        SendClientMessage(playerid, COLOR_YELLOW, "Turf ini sudah dimiliki gang kamu.");
        return 0;
    }
    if (TurfWarState[territoryIndex] != TURF_WAR_STATE_NONE)
    {
        SendClientMessage(playerid, COLOR_RED, "Turf ini sedang dalam war.");
        return 0;
    }
    if (TurfWarCooldownUntil[territoryIndex] > gettime())
    {
        new left = TurfWarCooldownUntil[territoryIndex] - gettime();
        new msg[144];
        format(msg, sizeof(msg), "Turf ini cooldown. Tunggu %d detik lagi.", left);
        SendClientMessage(playerid, COLOR_YELLOW, msg);
        return 0;
    }
    return 1;
...
stock CancelPlayerTurfHold(playerid, const reason[] = "")
{
    if (PlayerTurfHoldActive[playerid])
    {
        PlayerTurfHoldActive[playerid] = 0;
        PlayerTurfHoldTerritory[playerid] = -1;
        PlayerTurfHoldGang[playerid] = 0;
        PlayerTurfHoldStarted[playerid] = 0;
        if (strlen(reason) > 0) SendClientMessage(playerid, COLOR_YELLOW, reason);
    }
    return 1;
}

stock TryStartTurfChallengeHold(playerid)
{
    new territoryIndex = GetPlayerCurrentTerritory(playerid);
    if (territoryIndex == -1) return 0;
    if (!CanPlayerChallengeTurf(playerid, territoryIndex)) return 0;

    PlayerTurfHoldActive[playerid] = 1;
    PlayerTurfHoldTerritory[playerid] = territoryIndex;
...
public CompleteTurfChallengeHold(playerid, territoryIndex, gangid, startTick)
{
    ...
    if (!IsPlayerInsideTerritory(playerid, territoryIndex))
    {
        CancelPlayerTurfHold(playerid, "Challenge turf dibatalkan karena kamu keluar area.");
        return 1;
    }
...
stock StartTurfWar(playerid, territoryIndex)
{
    if (!CanPlayerChallengeTurf(playerid, territoryIndex)) return 0;
...
stock StopTurfWarFlash(territoryIndex)
{
    if (!IsValidTerritoryIndex(territoryIndex) || TerritoryZone[territoryIndex] == -1) return 0;
    for (new i = 0; i < MAX_PLAYERS; i++)
    {
        if (IsPlayerConnected(i)) GangZoneStopFlashForPlayer(i, TerritoryZone[territoryIndex]);
    }
    return 1;
}

stock ResetTurfWarState(territoryIndex)
{
    if (!IsValidTerritoryIndex(territoryIndex)) return 0;
    ...
    TurfWarCaptureSecondsLeft[territoryIndex] = 0;
    TurfWarGraceSecondsLeft[territoryIndex] = 0;
    RefreshTerritoryZoneForAll(territoryIndex);
    return 1;
}

stock SetTerritoryOwnerFromGang(territoryIndex, gangid)
{
    if (!IsValidTerritoryIndex(territoryIndex) || !IsPresetGangID(gangid)) return 0;
    new gangIndex = GetPresetGangIndexByID(gangid);
    TerritoryOwnerGangID[territoryIndex] = gangid;
    TerritoryOwnerColor[territoryIndex] = PresetGangColor[gangIndex];
    format(TerritoryOwnerName[territoryIndex], 64, "%s", PresetGangName[gangIndex]);

    new query[512];
    mysql_format(g_SQL, query, sizeof(query), "UPDATE gang_territories SET owner_gang_id=%d, owner_gang_name='%e', owner_color=%d, updated_at=NOW() WHERE territory_index=%d LIMIT 1", gangid, TerritoryOwnerName[territoryIndex], TerritoryOwnerColor[territoryIndex], territoryIndex + 1);
    mysql_tquery(g_SQL, query);
    RebuildTerritoryZone(territoryIndex);
    return 1;
}

stock RewardGangTurf(gangid, reward, reputation)
{
    if (gangid <= 0) return 0;
    new query[256];
    mysql_format(g_SQL, query, sizeof(query), "UPDATE gangs SET bank_money=bank_money+%d, reputation=reputation+%d WHERE id=%d LIMIT 1", reward, reputation, gangid);
    mysql_tquery(g_SQL, query);
    return 1;
}


stock GetAutoGangRankFromRespect(respect)
{
    if (respect >= 600) return GANG_RANK_OG;
    if (respect >= 250) return GANG_RANK_ENFORCER;
    if (respect >= 100) return GANG_RANK_SOLDIER;
    return GANG_RANK_MEMBER;
}

stock SaveOnlineGangRank(playerid)
{
    if (PlayerGangID[playerid] <= 0 || PlayerDBID[playerid] <= 0) return 0;
    new query[256];
    mysql_format(g_SQL, query, sizeof(query), "UPDATE gang_members SET rank_level=%d WHERE player_id=%d AND gang_id=%d LIMIT 1", PlayerGangRank[playerid], PlayerDBID[playerid], PlayerGangID[playerid]);
    mysql_tquery(g_SQL, query);
    return 1;
}

stock TryAutoPromoteByGangRespect(playerid)
{
    if (!IsPlayerConnected(playerid) || !PlayerLoggedIn[playerid] || PlayerGangID[playerid] <= 0) return 0;
    if (PlayerGangRank[playerid] >= GANG_RANK_LEADER) return 0;

    new suggestedRank = GetAutoGangRankFromRespect(PlayerGangRespect[playerid]);

    if (suggestedRank > PlayerGangRank[playerid])
    {
        PlayerGangRank[playerid] = suggestedRank;
        SaveOnlineGangRank(playerid);

        new rankName[32];
        new msg[144];
        GetGangRankName(suggestedRank, rankName, sizeof(rankName));
        format(msg, sizeof(msg), "Gang respect naik. Rank kamu sekarang: %s.", rankName);
        SendClientMessage(playerid, COLOR_GREEN, msg);
        return 1;
    }
...
stock AddGangMemberContribution(playerid, gangid, respect, captureInc, defendInc, warInc)
{
    if (!IsPlayerConnected(playerid) || !PlayerLoggedIn[playerid]) return 0;
    if (PlayerGangID[playerid] != gangid || gangid <= 0) return 0;

    new pname[MAX_PLAYER_NAME];
    GetPlayerName(playerid, pname, sizeof(pname));

    PlayerGangRespect[playerid] += respect;
    PlayerGangCaptures[playerid] += captureInc;
    PlayerGangDefends[playerid] += defendInc;
    PlayerGangWars[playerid] += warInc;
    ...
    mysql_format(
        ...
        query,
        sizeof(query),
        "INSERT INTO gang_member_stats (gang_id, player_id, player_name, respect, captures, defends, wars_participated, last_activity_at) VALUES (%d, %d, '%e', %d, %d, %d, %d, NOW()) ON DUPLICATE KEY UPDATE player_name=VALUES(player_name), respect=respect+%d, captures=captures+%d, defends=defends+%d, wars_participated=wars_participated+%d, last_activity_at=NOW(), updated_at=NOW()",
        gangid,
        PlayerDBID[playerid],
        pname,
        respect,
    ...
    TryAutoPromoteByGangRespect(playerid);
    return 1;
}

stock ProcessGangPromote(playerid, targetid)
{
    if (!IsGangLeader(playerid))
    {
        SendClientMessage(playerid, COLOR_RED, "Hanya Gang Boss yang bisa promote member.");
        return 0;
    }
    ...
    if (targetid == playerid)
    {
        SendClientMessage(playerid, COLOR_RED, "Tidak bisa promote diri sendiri.");
        return 0;
    }
    ...
    if (PlayerGangRank[targetid] >= GANG_RANK_OG)
    {
        SendClientMessage(playerid, COLOR_RED, "Gang Boss hanya bisa promote sampai OG. Gang Boss ditentukan Owner server.");
        return 0;
    }

    PlayerGangRank[targetid]++;
    SaveOnlineGangRank(targetid);

    new rankName[32], msg[160], targetName[MAX_PLAYER_NAME];
    GetPlayerName(targetid, targetName, sizeof(targetName));
    GetGangRankName(PlayerGangRank[targetid], rankName, sizeof(rankName));
    format(msg, sizeof(msg), "%s dipromote menjadi %s.", targetName, rankName);
    SendMessageToGang(PlayerGangID[playerid], COLOR_GREEN, msg);
    return 1;
}

stock ProcessGangDemote(playerid, targetid)
{
    if (!IsGangLeader(playerid))
    {
        SendClientMessage(playerid, COLOR_RED, "Hanya Gang Boss yang bisa demote member.");
        return 0;
    }
    ...
    if (targetid == playerid)
    {
        SendClientMessage(playerid, COLOR_RED, "Tidak bisa demote diri sendiri.");
        return 0;
    }
    ...
    if (PlayerGangRank[targetid] >= PlayerGangRank[playerid])
    {
        SendClientMessage(playerid, COLOR_RED, "Tidak bisa demote rank yang sama/lebih tinggi.");
        return 0;
    }

    if (PlayerGangRank[targetid] <= GANG_RANK_MEMBER)
    {
        SendClientMessage(playerid, COLOR_RED, "Target sudah berada di rank terendah.");
        return 0;
    }
    ...
    GetPlayerName(targetid, targetName, sizeof(targetName));
    GetGangRankName(PlayerGangRank[targetid], rankName, sizeof(rankName));
    format(msg, sizeof(msg), "%s didemote menjadi %s.", targetName, rankName);
    SendMessageToGang(PlayerGangID[playerid], COLOR_YELLOW, msg);
    return 1;
}

stock ShowGangRankInfoDialog(playerid)
{
    new body[1024];
    strcat(body, "Gang rank progression:\n\n", sizeof(body));
    strcat(body, "1. Member - Rank awal gang.\n", sizeof(body));
    strcat(body, "2. Soldier - Auto pada 100 respect.\n", sizeof(body));
    strcat(body, "3. Enforcer - Auto pada 250 respect.\n", sizeof(body));
    strcat(body, "4. OG - Auto pada 600 respect.\n", sizeof(body));
    strcat(body, "5. Gang Boss - Hanya ditetapkan Owner server.\n\n", sizeof(body));
    strcat(body, "Respect didapat dari capture/defend turf dan aktivitas gang.\n", sizeof(body));
    strcat(body, "Gang Boss bisa /gangpromote dan /gangdemote member sampai OG.", sizeof(body));

    ShowPlayerDialog(playerid, DIALOG_INFO, DIALOG_STYLE_MSGBOX, "Gang Rank Info", body, "OK", "");
    return 1;
}

stock ShowMyGangRespectDialog(playerid)
{
    if (PlayerGangID[playerid] <= 0)
    {
        SendClientMessage(playerid, COLOR_RED, "Kamu belum bergabung dengan gang.");
        return 0;
    }
    ...
    format(
        ...
        PlayerGangRespect[playerid],
        PlayerGangCaptures[playerid],
        PlayerGangDefends[playerid],
        PlayerGangWars[playerid]
    );

    ShowPlayerDialog(playerid, DIALOG_INFO, DIALOG_STYLE_MSGBOX, "My Gang Respect", body, "OK", "");
    return 1;
}

stock RewardGangMembersInside(gangid, territoryIndex, cash, xp, respect = 0, captureInc = 0, defendInc = 0, warInc = 0)
{
    if (gangid <= 0) return 0;
    new msg[144];
    for (new i = 0; i < MAX_PLAYERS; i++)
    {
        if (!IsPlayerConnected(i) || !PlayerLoggedIn[i]) continue;
        if (PlayerGangID[i] != gangid) continue;
        if (!IsPlayerInsideTerritory(i, territoryIndex)) continue;
        GivePlayerCash(i, cash);
        GivePlayerXPEx(i, xp);
        SavePlayerData(i);
        if (respect > 0 || captureInc > 0 || defendInc > 0 || warInc > 0)
        {
            AddGangMemberContribution(i, gangid, respect, captureInc, defendInc, warInc);
        }
        format(msg, sizeof(msg), "Turf reward: +$%d +%d XP +%d respect.", cash, xp, respect);
        SendClientMessage(i, COLOR_GREEN, msg);
    }
    return 1;
}

stock CompleteTurfCapture(territoryIndex)
{
    new attacker = TurfWarAttackerGangID[territoryIndex];
    if (attacker <= 0) return 0;
    new attackerName[64];
    GetGangNameByID(attacker, attackerName, sizeof(attackerName));
    SetTerritoryOwnerFromGang(territoryIndex, attacker);
    RewardGangTurf(attacker, TURF_CAPTURE_GANG_REWARD, 10);
    RewardGangMembersInside(attacker, territoryIndex, TURF_CAPTURE_PLAYER_REWARD, TURF_CAPTURE_XP, 25, 1, 0, 1);
    new msg[192];
...
stock CompleteTurfDefend(territoryIndex)
{
    new defender = TurfWarDefenderGangID[territoryIndex];
    if (defender <= 0)
    {
        new msgNeutral[160];
        format(msgNeutral, sizeof(msgNeutral), "[TURF CANCELLED] %s tetap Neutral karena attacker gagal bertahan.", TerritoryName[territoryIndex]);
        SendClientMessageToAll(COLOR_YELLOW, msgNeutral);
        TurfWarCooldownUntil[territoryIndex] = gettime() + 180;
        ResetTurfWarState(territoryIndex);
        return 1;
    }
    RewardGangTurf(defender, TURF_DEFEND_GANG_REWARD, 5);
    RewardGangMembersInside(defender, territoryIndex, TURF_DEFEND_PLAYER_REWARD, TURF_DEFEND_XP, 15, 0, 1, 1);
    new defenderName[64];
    GetGangNameByID(defender, defenderName, sizeof(defenderName));
    new msg[192];
...
stock CancelTurfWar(territoryIndex, const reason[])
{
    if (!IsValidTerritoryIndex(territoryIndex) || TurfWarState[territoryIndex] == TURF_WAR_STATE_NONE) return 0;
    new msg[192];
...
stock ShowGangMenuDialog(playerid)
{
    ...
    if (PlayerGangID[playerid] <= 0)
    {
        ...
        strcat(body, "Gang Rank Info\n", sizeof(body));
        strcat(body, "Recent Turf Logs", sizeof(body));
    }
    else
    {
        strcat(body, "Gang Info\n", sizeof(body));
...
stock SaveGangHQInterior(gangid)
{
    ...
    new query[1024];
    mysql_format(g_SQL, query, sizeof(query), "REPLACE INTO gang_hq_interiors (gang_id, gang_name, interior_id, virtual_world, int_x, int_y, int_z, int_a, exit_x, exit_y, exit_z, exit_a, enabled) VALUES (%d, '%e', %d, %d, %f, %f, %f, %f, %f, %f, %f, %f, %d)", gangid, PresetGangName[gangIndex], GangHQInteriorID[gangIndex], GangHQInteriorVirtualWorld[gangIndex], GangHQInteriorX[gangIndex], GangHQInteriorY[gangIndex], GangHQInteriorZ[gangIndex], GangHQInteriorA[gangIndex], GangHQInteriorExitX[gangIndex], GangHQInteriorExitY[gangIndex], GangHQInteriorExitZ[gangIndex], GangHQInteriorExitA[gangIndex], GangHQInteriorEnabled[gangIndex]);
    mysql_tquery(g_SQL, query);
    return 1;
}
...
stock SetGangInteriorFromPlayer(playerid, gangid)
{
    ...
    GangHQInteriorExitX[gangIndex] = GangHQX[gangIndex];
    GangHQInteriorExitY[gangIndex] = GangHQY[gangIndex];
    GangHQInteriorExitZ[gangIndex] = GangHQZ[gangIndex];
    GangHQInteriorExitA[gangIndex] = 0.0;
...
stock ShowGangHQUtilityInfo(playerid)
{
    ...
    format(msg, sizeof(msg), "Gang: %s | HQ: %.2f %.2f %.2f", PresetGangName[gangIndex], GangHQX[gangIndex], GangHQY[gangIndex], GangHQZ[gangIndex]);
    SendClientMessage(playerid, COLOR_WHITE, msg);
    SendClientMessage(playerid, COLOR_WHITE, "/enterganghq = masuk interior HQ jika dekat HQ.");
    SendClientMessage(playerid, COLOR_WHITE, "/exitganghq = keluar interior HQ.");
    SendClientMessage(playerid, COLOR_WHITE, "/gangstash = weapon stash, bisa dari HQ luar atau interior HQ.");
    return 1;
}
...
stock ShowDynamicLocationListDialog(playerid)
{
    ...
    if (DynamicLocationCount == 0)
    {
        ShowPlayerDialog(playerid, DIALOG_DYNAMIC_LOCATION_INFO, DIALOG_STYLE_MSGBOX, "Dynamic Locations", "Belum ada dynamic location aktif.", "Back", "Close");
        return 1;
    }

    format(body, sizeof(body), "ID\tType\tIcon\tRadius\tName\n");

    for (new i = 0; i < DynamicLocationCount; i++)
    {
        format(line, sizeof(line), "%d\t%s\t%d\t%.1f\t%s\n", DynamicLocationDBID[i], DynamicLocationType[i], DynamicLocationMapIcon[i], DynamicLocationRadius[i], DynamicLocationName[i]);
        strcat(body, line, sizeof(body));
    }

    ShowPlayerDialog(playerid, DIALOG_LOC_LIST, DIALOG_STYLE_TABLIST_HEADERS, "Select Dynamic Location", body, "Select", "Back");
    return 1;
}

stock ShowDynamicLocationSelectInput(playerid)
{
    ShowPlayerDialog(
        playerid,
        DIALOG_LOC_SELECT_INPUT,
        DIALOG_STYLE_INPUT,
        "Select Dynamic Location",
        "Masukkan ID location yang ingin diedit.\n\nGunakan /loclist atau menu List untuk melihat ID aktif.",
        "Select",
        "Back"
    );
    return 1;
...
stock ShowDynamicLocationActionMenu(playerid, locId)
{
    new idx = FindDynamicLocationIndexByDBID(locId);
    if (idx == -1)
    {
        SendClientMessage(playerid, COLOR_RED, "Dynamic location ID tidak ditemukan atau nonaktif.");
        ShowDynamicLocationEditorMenu(playerid);
        return 0;
    }
    ...
    format(title, sizeof(title), "Location ID %d", locId);
    format(body, sizeof(body), "Info\nGoto\nMove to My Position\nRename Location\nEdit Type\nEdit Label\nEdit Radar Icon\nEdit Pickup / Marker\nEdit Object Visual\nEdit ALT Radius\nEdit Interior / Virtual World\nEnable / Disable\nDelete Location\nReload All\nBack");

    ShowPlayerDialog(playerid, DIALOG_LOC_ACTION_MENU, DIALOG_STYLE_LIST, title, body, "Select", "Close");
    return 1;
}

stock ShowDynamicLocationEditorInfoDialog(playerid, locId)
{
    new idx = FindDynamicLocationIndexByDBID(locId);
    if (idx == -1)
    {
    ...
    format(
        ...
        sizeof(body),
        "ID: %d\nType: %s\nName: %s\nLabel: %s\nInterior: %d\nVirtual World: %d\nPosition: %.2f %.2f %.2f\nAngle: %.2f\nMap Icon: %d\nPickup Model: %d\nObject Model: %d\nLinked Object ID: %d\nALT Radius: %.1f\nEnabled: %d",
        DynamicLocationDBID[idx],
        DynamicLocationType[idx],
        DynamicLocationName[idx],
        DynamicLocationLabelText[idx],
        DynamicLocationInterior[idx],
        DynamicLocationVirtualWorld[idx],
        DynamicLocationX[idx],
        DynamicLocationY[idx],
        DynamicLocationZ[idx],
        DynamicLocationA[idx],
        DynamicLocationMapIcon[idx],
        DynamicLocationPickupModel[idx],
        DynamicLocationObjectModel[idx],
        DynamicLocationLinkedObjectDBID[idx],
        DynamicLocationRadius[idx],
        DynamicLocationEnabled[idx]
    );

    ShowPlayerDialog(playerid, DIALOG_DYNAMIC_LOCATION_INFO, DIALOG_STYLE_MSGBOX, "Dynamic Location Info", body, "Back", "Close");
    return 1;
}

stock GotoDynamicLocationByID(playerid, locId)
{
    new idx = FindDynamicLocationIndexByDBID(locId);
    if (idx == -1)
    {
        SendClientMessage(playerid, COLOR_RED, "Dynamic location tidak ditemukan/aktif.");
        return 0;
    }

    SetPlayerInterior(playerid, DynamicLocationInterior[idx]);
    SetPlayerVirtualWorld(playerid, DynamicLocationVirtualWorld[idx]);
    SetPlayerPos(playerid, DynamicLocationX[idx], DynamicLocationY[idx], DynamicLocationZ[idx] + 1.0);
    SetPlayerFacingAngle(playerid, DynamicLocationA[idx]);
    SendClientMessage(playerid, COLOR_GREEN, "Teleport ke dynamic location berhasil.");
    return 1;
}

stock MoveDynamicLocationToPlayer(playerid, locId)
{
    if (FindDynamicLocationIndexByDBID(locId) == -1)
    {
        SendClientMessage(playerid, COLOR_RED, "Dynamic location tidak ditemukan/aktif.");
...
stock ShowDynamicLocationRenameInput(playerid, locId)
{
    new idx = FindDynamicLocationIndexByDBID(locId);
    if (idx == -1)
    {
        SendClientMessage(playerid, COLOR_RED, "Dynamic location ID tidak ditemukan.");
        ShowDynamicLocationEditorMenu(playerid);
        return 0;
    }
    ...
    new body[256];
    format(body, sizeof(body), "Nama sekarang: %s\n\nMasukkan nama/display name baru.", DynamicLocationName[idx]);
    ShowPlayerDialog(playerid, DIALOG_LOC_RENAME_INPUT, DIALOG_STYLE_INPUT, "Rename Dynamic Location", body, "Save", "Back");
    return 1;
}

stock ShowDynamicLocationTypeInput(playerid, locId)
{
    new idx = FindDynamicLocationIndexByDBID(locId);
    if (idx == -1)
    {
    ...
    new body[384];
    format(body, sizeof(body), "Type sekarang: %s\n\nMasukkan type baru.\nContoh: atm, dealer, ammunation, gang_hq, job, race, business, house, interior, public_interior, pickup", DynamicLocationType[idx]);
    ShowPlayerDialog(playerid, DIALOG_LOC_TYPE_INPUT, DIALOG_STYLE_INPUT, "Edit Location Type", body, "Save", "Back");
    return 1;
}

stock ShowDynamicLocationLabelInput(playerid, locId)
{
    new idx = FindDynamicLocationIndexByDBID(locId);
    if (idx == -1)
    {
    ...
    new body[512];
    format(body, sizeof(body), "Label sekarang:\n%s\n\nMasukkan label baru.\nGunakan off/hide untuk sembunyikan.\nGunakan auto/default untuk label otomatis.", DynamicLocationLabelText[idx]);
    ShowPlayerDialog(playerid, DIALOG_LOC_LABEL_INPUT, DIALOG_STYLE_INPUT, "Edit Location Label", body, "Save", "Back");
    return 1;
}

stock ShowDynamicLocationIconInput(playerid, locId)
{
    new idx = FindDynamicLocationIndexByDBID(locId);
    if (idx == -1)
    {
    ...
    new body[384];
    format(body, sizeof(body), "Map icon sekarang: %d\n\nMasukkan ID radar/map icon baru.\nPreset cepat:\nATM/Bank 52\nDealer 55\nJob 51\nRace 53\nAmmu-Nation 6\nGang HQ 19\nGunakan 0 untuk hilangkan icon.", DynamicLocationMapIcon[idx]);
    ShowPlayerDialog(playerid, DIALOG_LOC_ICON_INPUT, DIALOG_STYLE_INPUT, "Edit Radar Icon", body, "Save", "Back");
    return 1;
}

stock ShowDynamicLocationPickupInput(playerid, locId)
{
    new idx = FindDynamicLocationIndexByDBID(locId);
    if (idx == -1)
    {
    ...
    new body[384];
    format(body, sizeof(body), "Pickup/marker sekarang: %d\n\nMasukkan model pickup baru.\nContoh: 1239 info marker.\nGunakan 0 untuk hapus pickup/marker.", DynamicLocationPickupModel[idx]);
    ShowPlayerDialog(playerid, DIALOG_LOC_PICKUP_INPUT, DIALOG_STYLE_INPUT, "Edit Pickup / Marker", body, "Save", "Back");
    return 1;
}

stock ShowDynamicLocationObjectInput(playerid, locId)
{
    new idx = FindDynamicLocationIndexByDBID(locId);
    if (idx == -1)
    {
    ...
    new body[384];
    format(body, sizeof(body), "Object visual sekarang: %d\n\nMasukkan model object visual baru.\nContoh ATM object: 2942.\nGunakan 0 untuk hapus object visual location.", DynamicLocationObjectModel[idx]);
    ShowPlayerDialog(playerid, DIALOG_LOC_OBJECT_INPUT, DIALOG_STYLE_INPUT, "Edit Object Visual", body, "Save", "Back");
    return 1;
}

stock ShowDynamicLocationRadiusInput(playerid, locId)
{
    new idx = FindDynamicLocationIndexByDBID(locId);
    if (idx == -1)
    {
    ...
    new body[256];
    format(body, sizeof(body), "ALT radius sekarang: %.1f\n\nMasukkan radius baru 1 sampai 50.\nContoh: 3 atau 5", DynamicLocationRadius[idx]);
    ShowPlayerDialog(playerid, DIALOG_LOC_RADIUS_INPUT, DIALOG_STYLE_INPUT, "Edit ALT Radius", body, "Save", "Back");
    return 1;
}

stock ShowDynamicLocationInteriorInput(playerid, locId)
{
    new idx = FindDynamicLocationIndexByDBID(locId);
    if (idx == -1)
    {
    ...
    new body[384];
    format(body, sizeof(body), "Interior sekarang: %d\nVirtual World sekarang: %d\n\nMasukkan format:\ninterior virtual_world\n\nContoh public world: 0 0\nContoh custom world: 0 50001", DynamicLocationInterior[idx], DynamicLocationVirtualWorld[idx]);
    ShowPlayerDialog(playerid, DIALOG_LOC_INTERIOR_INPUT, DIALOG_STYLE_INPUT, "Edit Interior / Virtual World", body, "Save", "Back");
    return 1;
}

stock ShowDynamicLocationEnableMenu(playerid, locId)
{
    if (FindDynamicLocationIndexByDBID(locId) == -1)
    {
        SendClientMessage(playerid, COLOR_RED, "Dynamic location ID tidak ditemukan.");
    ...
    PlayerEditingLocationID[playerid] = locId;
    ShowPlayerDialog(playerid, DIALOG_LOC_ENABLE_MENU, DIALOG_STYLE_LIST, "Enable / Disable Location", "Enable Location\nDisable / Hide Location", "Select", "Back");
    return 1;
}

stock ShowDynamicLocationDeleteConfirm(playerid, locId)
{
    new idx = FindDynamicLocationIndexByDBID(locId);
    if (idx == -1)
    {
    ...
    format(
        body,
        sizeof(body),
        "Hapus dynamic location ID %d?\n\nType: %s\nName: %s\n\nIni akan menghapus ALT interaction, radar icon, marker/pickup, dan 3D label location.\nObject yang ditautkan tidak otomatis dihapus kecuali memakai object purge.",
        locId,
        DynamicLocationType[idx],
        DynamicLocationName[idx]
    );

    ShowPlayerDialog(playerid, DIALOG_LOC_DELETE_CONFIRM, DIALOG_STYLE_MSGBOX, "Delete Dynamic Location", body, "Delete", "Back");
    return 1;
}
...
stock InitWorldMarkerArrays()
{
    ...
    for (new i = 0; i < MAX_PRESET_GANGS; i++)
    {
        ...
        GangHQInteriorExitPickup[i] = -1;
        GangHQInteriorExitLabel[i] = Text3D:INVALID_3DTEXT_ID;
    }
...
stock DestroyWorldInteractionMarkers()
{
    ...
    DestroyAllGangHQInteriorRuntime();
...
public OnPlayerPickUpPickup(playerid, pickupid)
{
    ...
    for (new g = 0; g < MAX_PRESET_GANGS; g++)
    {
        if (GangHQPickup[g] != -1 && pickupid == GangHQPickup[g])
        {
            if (IsPlayerInGangHQPickupCooldown(playerid))
            {
                return 1;
            ...
            EnterPlayerGangHQInterior(playerid, PresetGangID[g]);
            return 1;
        }
    }
...
public OnPlayerGangLoaded(playerid)
{
    ...
    cache_get_value_name_int(0, "respect", PlayerGangRespect[playerid]);
...
public OnPlayerCommandText(playerid, cmdtext[])
{
    ...
    if (strfind(cmdtext, "/cancelturfwar ", true) == 0)
    {
        if (!IsAdminLevel(playerid, ADMIN_OWNER))
        {
            SendClientMessage(playerid, COLOR_RED, "Hanya Owner yang bisa cancel turf war.");
            return 1;
        }
    ...
    if (!strcmp(cmdtext, "/setganginterior", true))
    {
        SendClientMessage(playerid, COLOR_YELLOW, "Gunakan: /setganginterior [gang_id]");
        return 1;
    }

    if (strfind(cmdtext, "/ganginteriorinfo ", true) == 0)
    {
        new gangStr[16];
        if (!GetOneParam(cmdtext[18], gangStr, sizeof(gangStr)) || !IsNumericString(gangStr))
        {
            SendClientMessage(playerid, COLOR_YELLOW, "Gunakan: /ganginteriorinfo [gang_id]");
            return 1;
        }
    ...
    if (!strcmp(cmdtext, "/gangpromote", true))
    {
        SendClientMessage(playerid, COLOR_YELLOW, "Gunakan: /gangpromote [playerid]");
        return 1;
    }

    if (strfind(cmdtext, "/gangdemote ", true) == 0)
    {
        new targetStr[16];
        if (!GetOneParam(cmdtext[12], targetStr, sizeof(targetStr)) || !IsNumericString(targetStr))
        {
            SendClientMessage(playerid, COLOR_YELLOW, "Gunakan: /gangdemote [playerid]");
            return 1;
        }
    ...
    if (!strcmp(cmdtext, "/locmenu", true) || !strcmp(cmdtext, "/locedit", true) || !strcmp(cmdtext, "/locationmenu", true))
    {
        ShowDynamicLocationEditorMenu(playerid);
        return 1;
    }
