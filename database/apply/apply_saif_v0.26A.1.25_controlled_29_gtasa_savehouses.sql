...
DELIMITER //
CREATE PROCEDURE saif_apply_gtasa29_savehouses_v026A125()
main: BEGIN
    ...
    DECLARE v_public_overflow INT DEFAULT 0;
    ...
    SET v_public_overflow=GREATEST(0,v_public_icons+v_hospital_fallback-91);
    IF v_public_overflow<>0 THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Public map icon count exceeds reserved 91-slot budget.'; END IF;
    ...
    VALUES
      (v_apply_key,v_archive_session_id,v_resolver_session_id,'applying',v_catalog_total_before,v_catalog_active_before,v_ownership_before,v_preserve_rows,v_mapped_rows,
       'Controlled apply: preserve explicitly protected legacy definitions, insert exactly 29 canonical GTA SA savehouses, map only explicit mapped ownership rows.');
    ...
    SET apply_status='complete',legacy_rows_disabled=v_disabled_count,canonical_rows_inserted=v_inserted_count,
        ...
        notes=CONCAT('Complete. Disabled ',v_disabled_count,' non-preserved legacy definitions; inserted 29 canonical GTA SA savehouses; preserved ownership rows ',v_preserve_rows,'; mapped ownership rows ',v_mapped_rows,'.')
    WHERE id=v_apply_session_id;
    ...
    SELECT v_apply_session_id apply_session_id,v_archive_session_id archive_session_id,v_source_tag source_tag,
           ...
           v_preserve_rows preserve_legacy_rows,v_mapped_rows mapped_ownership_rows,v_active_after active_after,'COMPLETE' apply_status;
END//
DELIMITER ;
CALL saif_apply_gtasa29_savehouses_v026A125();
DROP PROCEDURE IF EXISTS saif_apply_gtasa29_savehouses_v026A125;
